...
class DoubleIntegratorEnv(gym.Env):
    ...
    def _render(self):
        ...
        self.axs[self.reward_plot_idx].legend(
            [
                f"Reward Per Episode: {np.sum(self.reward_array)/self.number_episodes:.3f}"
            ],
            loc="center right",
    ...
    def reset(self, init_state: State = None, reset_data=True, seed=None, options=None):
        ...
        if init_state is not None:
            ...
            self.state = State(
                station=np.random.uniform(-10.0, 10.0),
                speed=np.random.uniform(-10.0, 10.0),
            )
    ...
    def render(self, file_name=None):
        if self.render_mode == "save" and self.time >= self.max_time:
            self._render()
